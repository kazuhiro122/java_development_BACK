window.onload = function () {
  var cart_btns = document.querySelectorAll('js_cart_btn'),//カートボタンに「js_cart_btn」を付与
  cart_cnt_icon = document.getElementById('js_cart_cnt'),//カートの個数アイコン カートアイコンに「js_cart_cnt」を付与
  ...
  if (items) {
    ...
    if(items.length != 0){
      cart_cnt_icon.parentNode.classList.remove('hidden');//カートアイコンの数字を表示 cart_cnt hiddenクラス
      cart_cnt_icon.innerHTML = cart_cnt;
    }
  }

  // カートボタンを押した際の処理
  cart_btns.forEach(function (cart_btn,index) {//cart_btnをカートに追加ボタンに付与
    cart_btn.addEventListener('click',function () {
      ...
      if (clicked.indexOf(index) >= 0) {
        ...
        var name = cart_btn.dataset.name,//商品の名前を取得 商品にdate-name = "商品名"今回で言うとデータベースから取得
        price = Number(cart_btn.dataset.price);//商品の値段を取得 商品にdate-price = "料金"今回で言うとデータベースから取得
